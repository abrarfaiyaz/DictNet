...
inputs = Input(shape=(dwi.shape[3],))
t2in=Input(shape=(1,))
FAin=Input(shape=(1,))
I = Sequential()
...
for iMask in range(len(allTestDwiNames)):
    ...
    dwiTest = np.zeros([nVox, dwi.shape[3]]) #dwiTest = np.zeros([nVox, dwi.shape[3]*neighbor_size])
    t2Test=np.zeros([nVox, 1])
    FATest=np.zeros([nVox, 1])
        ...
        for j in range(dwi.shape[1]):
            for k in range(dwi.shape[2]):
                if mask[i,j,k] > 0:
                    ...
                    t2Test[nVox,:]=t2[i,j,k]
                    FATest[nVox,:]=FA[i,j,k]
                    voxelList[nVox,0] = i
                    voxelList[nVox,1] = j
                    voxelList[nVox,2] = k
                    nVox = nVox + 1
