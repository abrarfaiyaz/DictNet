...
t2Training = np.zeros([nVox, neighbor_size])
FATraining = np.zeros([nVox, neighbor_size])
        ...
        for j in range(dwi.shape[1]):
            for k in range(dwi.shape[2]):
                if mask[i,j,k] > 0:
                    ...
                    tcount=0 # Addressing boundary or mask_boundary
                    for ii in [-1,0,1]:
                        for jj in [-1,0,1]:
                            for kk in [-1,0,1]:
                                if i + ii >= 0 and i + ii < dwi.shape[0] and j + jj >= 0 and j + jj < dwi.shape[1] \
                                    ...
                                    t2Training[nVox,tcount] = t2[i+ii, j+jj, k+kk]
                                    FATraining[nVox,tcount] = FA[i+ii, j+jj, k+kk]
                                else: #incase of the voxels at boundary 
                                    dwiTraining[nVox, ((ii+1)*9 + (jj+1)*3 + (kk+1))*dwi.shape[3]:((ii+1)*9 + (jj+1)*3 + (kk+1) + 1)*dwi.shape[3]] = dwi[i, j, k, :]
                                    t2Training[nVox,tcount] = t2[i, j, k]
                                    FATraining[nVox,tcount] = FA[i, j, k]
                                tcount=tcount+1
...
save=1
if save==1:
    clf.save_weights("DictNet2_"+ protocol +"_"+str(seedvalue)+".h5")
